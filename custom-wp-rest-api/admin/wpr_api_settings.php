...
if(isset($_POST['wpr_save_settings'])){
  ...
  $nonce = isset($_REQUEST['udate_flds_non']) ? sanitize_text_field($_REQUEST['udate_flds_non']) : '';
  if ( ! wp_verify_nonce( $nonce, 'udate_flds' ) ) {
    ...
    if(isset($_POST['wpr_set_recent_activity_dur'])){
      $_get_settings['wpr_set_recent_activity_dur'] = sanitize_text_field($_POST['wpr_set_recent_activity_dur']);
    }
...
$wpr_set_req_check = (isset($wpr__settings['wpr_set_req_check']) && esc_attr($wpr__settings['wpr_set_req_check']) == 1) ? 'checked' : '';
$wpr_set_ver = isset($wpr__settings['wpr_set_ver']) ? esc_attr($wpr__settings['wpr_set_ver']) : '1';
$wpr_set_end_slug = isset($wpr__settings['wpr_set_end_slug']) ? esc_attr($wpr__settings['wpr_set_end_slug']) : 'wcra';
$wpr_set_auth_check = (isset($wpr__settings['wpr_set_auth_check']) && esc_attr($wpr__settings['wpr_set_auth_check']) == 1) ? 'checked' : '';
$wpr_set_enb_api = (isset($wpr__settings['wpr_set_enb_api']) && esc_attr($wpr__settings['wpr_set_enb_api']) == 1) ? 'checked' : '';
$wpr_set_auth_check_by_admin = (isset($wpr__settings['wpr_set_auth_check_by_admin']) && esc_attr($wpr__settings['wpr_set_auth_check_by_admin']) == 1) ? 'checked' : '';
$wpr_set_recent_activity_dur = isset($wpr__settings['wpr_set_recent_activity_dur']) ? esc_attr($wpr__settings['wpr_set_recent_activity_dur']) : 3;


 ?>
 <div class="gsr_back_body">
 <div class="wraparea">
 <h2><?php esc_html_e( 'Settings', 'custom-wp-rest-api' ); ?></h2>
<form action="" method="post">
<table class="form-table wpr_set_table_wrap">
  <input type="hidden" value="<?php echo esc_attr(wp_create_nonce( 'udate_flds' ));?>" name="udate_flds_non">
 <tr>
    <th class="row"><?php esc_html_e( 'Enable Api' , 'custom-wp-rest-api'); ?></th>
    <td>
      <label class="switch">
  ...
  <tr>
    <th class="row"><?php esc_html_e( "Enable Request's Log Capturing" , 'custom-wp-rest-api'); ?></th>
    <td>
     <label class="switch">
  ...
  <tr>
    <th class="row"><?php esc_html_e( 'Enable Api Authentication with Secret Key' , 'custom-wp-rest-api'); ?></th>
    <td>
      <label class="switch">
  ...
  <tr>
    <th class="row"><?php esc_html_e('Api Version' , 'custom-wp-rest-api'); ?></th>
    <td><input type="number" name="wpr_set_ver" value="<?php echo esc_attr($wpr_set_ver); ?>" min="1"></td>
  </tr>

  <tr>
    <th class="row"><?php esc_html_e('Api Namespace' , 'custom-wp-rest-api'); ?></th>
...
Then <strong>'wcra'</strong> is your NAMESAPCE. and it would be remain same on all the endpoint URL."><i class="fa fa-question-circle" aria-hidden="true"></i></a></td>
  ...
  <tr>
    <th class="row"><?php esc_html_e('Keep Recent Activity Log For' , 'custom-wp-rest-api'); ?></th>
    <td>
...
<p class="submit"><input name="wpr_save_settings" id="submit" class="button button-primary" value="<?php esc_html_e('Save Settings' , 'custom-wp-rest-api'); ?>" type="submit">
<input name="wpr_reset_settings" id="wpr_reset_settings" class="button button-primary" value="<?php esc_html_e('Reset Default Settings' , 'custom-wp-rest-api'); ?>" type="button"></p>
</form>
</div>
</div>
