...
<thead>
  ...
  <tbody>
   ...
   <?php if(is_array($_get_list_api_access) && count($_get_list_api_access)){
      foreach ($_get_list_api_access as $key => $value) {
        ...
        ?>
        <tr>
          <td><?php echo esc_attr($value->Fullname); ?></td>
          <td><?php echo esc_attr($value->Email); ?></td>
         <!--  <td><?php //echo $value->ApiSecret; ?></td> -->
          <td><?php echo esc_attr($value->CreatedAt); ?></td>
          <td><?php echo wp_kses_post($staus); ?></td>
