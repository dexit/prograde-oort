...
function wcra_api_walk_help_callback(){
 ...
 function wcra_authintication($secret){
 	...
 	$wpr_set_auth_check = isset($_get_settings['wpr_set_auth_check']) ? esc_attr($_get_settings['wpr_set_auth_check']) : 1;
...
function wcra_get_settings($key=''){
	...
	if($key){
		return isset($wpr__settings[$key]) ? $wpr__settings[$key] : '';
	}else{
		return is_array($wpr__settings) ? $wpr__settings : array();
	}
}
...
function wcra_get_end_url($base,$param=array()){
	$str = '';
	$_get_settings = wcra_get_settings();
	$my_namespace = isset($_get_settings['wpr_set_end_slug']) ? $_get_settings['wpr_set_end_slug'] : 'wcra';
	$my_version = isset($_get_settings['wpr_set_ver']) ? $_get_settings['wpr_set_ver'] : '1';
	$namespace = $my_namespace . '/v'. $my_version;
	$home_url = home_url().'/wp-json';
    $secret_key = (isset($param['secret_key']) && $param['secret_key'] != '') ? $param['secret_key'] : wcra_get_root_secret();
...
function wcra_get_root_secret(){
	$_get_settings = wcra_get_settings();
	$root_secret = isset($_get_settings['root_secret']) ? $_get_settings['root_secret'] : '';
	return $root_secret;
}
...
function wcra_delete_log($days=7){
	...
	$q = "SELECT count(*) from $cus_api_log where connectedAt <= '$set_border 11:59:59' ";
	// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table creation query, safe usage.
	...
	//return $get;
	$q1 = "DELETE from $cus_api_log where  connectedAt <= '$set_border 11:59:59' ";
	// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table creation query, safe usage.
