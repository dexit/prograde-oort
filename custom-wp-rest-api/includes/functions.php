...
function wcra_invalid_request(){
  ...
  if($actual_link == $link || $actual_link == $link.'/'){
    ...
    $create_log = wcra_create_api_log($secret_key,$error);
   // die;
    return $error;
    die;
  }
...
function wcra_get_basedata($base){
  global $wpdb;
  $tab = esc_sql($wpdb->prefix.WCRA_DB).'api_endpoints';
...
function wcra_add_settings_link( $links ) {
    $settings_link = '<a href="'.admin_url( "admin.php?page=wcra_api_endpoints" ).'">' . __( 'Settings' , 'custom-wp-rest-api' ) . '</a>';
    array_push( $links, $settings_link );
    return $links;
}

function wcra_activation_redirect( $plugin ) {
  wp_redirect( esc_url_raw( admin_url( 'admin.php?page=wcra_api_endpoints' ) ) );
  exit;
}
...
function wcra_save_recent_activity($data){
  ...
  $tobeinsert = array('notification' => $notification, 'date_time' => gmdate('Y-m-d H:i:s'));
...
function wcra_delete_recent_activity($days=0){
  ...
  $set_border = gmdate('Y-m-d', strtotime("-$days days"));
  $notification_log = $wpdb->prefix.WCRA_DB.'notification_log';
  $q = "SELECT count(*) from $notification_log where date_time <= '$set_border 11:59:59' ";
  // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table creation query, safe usage.
  $get = (int)$wpdb->get_var($q);
  if($get > 0 ){
  ...
  //return $get;
  $q1 = "DELETE from $notification_log where date_time <= '$set_border 11:59:59' ";
  // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table creation query, safe usage.
...
function wcra_admin_page_tabs( $current = 'wcra_new_api' ) {
    $tabs = array(
        ...
        'wcra_api_walk_help'  => __( '<i class="fa fa-question-circle" aria-hidden="true"></i>&nbsp;Walk Through', 'custom-wp-rest-api' ),
    );
    $html = '<h2 class="nav-tab-wrapper">';
    foreach( $tabs as $tab => $name ){
        $class = ( $tab == $current ) ? 'nav-tab-active' : '';
        $html .= '<a class="nav-tab navmar ' . esc_attr($class) . '" href="?page=' .esc_attr( $tab ). '">' . wp_kses_post($name) . '</a>';
    }
    $html .= '</h2>';
